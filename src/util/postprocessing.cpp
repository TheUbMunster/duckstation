...
bool PostProcessing::Apply(GPUTexture* final_target, GSVector4i final_rect, s32 orig_width, s32 orig_height,
                           s32 native_width, s32 native_height)
{
  ...
  for (const std::unique_ptr<Shader>& stage : s_stages)
  {
    ...
    if (!stage->Apply(input, is_final ? final_target : output, final_rect, orig_width, orig_height, native_width,
                      native_height, s_target_width, s_target_height))
    {
      return false;
    }
